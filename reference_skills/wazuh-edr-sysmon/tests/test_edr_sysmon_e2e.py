...
# Configuration (from environment variables only — no defaults for secrets)
import os
...
ITOP_USER = os.environ.get("ITOP_USER", "admin")
...
def indexer_request(endpoint, data=None):
...
def itop_request(operation, fields):
    ...
    payload = {"version": "1.4", "json_output": 1, "user": ITOP_USER,
               "password": ITOP_PASSWORD}
    payload.update(fields)

    body = f"version=1.4&json_output=1&json_data={urllib.request.quote(json.dumps(payload))}".encode()
...
def test_manager_status(token, result):
    ...
    running = resp.get("data", {}).get("running", "unknown")
    result.record("Wazuh Manager processes running", running == "yes")
...
def test_indexer_indices(result):
    ...
    resp = indexer_request("/_cat/indices")
    result.record("Indexer indices accessible", "error" not in resp)
...
def test_modular_isolation(result):
    ...
    if token:
        ...
        resp = wazuh_request(token, "GET", "/manager/info")
        has_version = "data" in resp and "version" in resp.get("data", {})
        result.record("Wazuh operates independently (modular)", has_version)
    else:
