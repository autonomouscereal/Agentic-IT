...
python "C:\Users\me\.agents\skills\server-manager\ssh_client.py" --server ai --command-file "$env:TEMP\switch_agentic_ops_route.sh"
```
...
`/opt/agentic-it/SOC_TESTING/soc-dashboard`. The live proxy is
`http://ai-proxy:4001` inside Docker and `http://127.0.0.1:4001` from
