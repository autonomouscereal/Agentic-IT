...
`scripts/itop_client.py` loads credentials from environment or the dashboard/iTop `.env` files. It sends both Basic Auth and `user`/`password` inside `json_data`; do not reintroduce hardcoded passwords.
...
The dashboard can create iTop `Incident` and `UserRequest` records without hardcoding org/caller IDs. It prefers `ITOP_DEFAULT_ORG_ID` and `ITOP_DEFAULT_CALLER_ID` when set, then resolves Organization `1` or the first Organization and a Person in that org. `ITOP_SECURITY_TEAM_ID` is optional and only used when valid.

Verified on 2026-05-12:

| Path | iTop object | Defaults |
|------|-------------|----------|
| `/api/tickets` UserRequest create | `UserRequest::169` | org `1`, caller `94`, team `65` |
| `/api/tickets` Incident create | `Incident::170` | org `1`, caller `94`, team `65`, impact `2`, urgency `2` |
