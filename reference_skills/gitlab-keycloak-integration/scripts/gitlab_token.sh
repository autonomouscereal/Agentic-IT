#!/usr/bin/env bash
# Shared GitLab PAT resolver for OIDC integration scripts. Reads env first,
# then optional token file, then the encrypted server-manager vault via credman.py.

load_gitlab_pat() {
    local vault_key="${1:-${GITLAB_PAT_VAULT_KEY:-gitlab_oidc_setup_pat}}"
