...
description: >
  Detailed troubleshooting guide for platform_demo_user login failures across all AI Server platforms.
...
**Date:** 2026-05-06
**Affected Account:** `platform_demo_user`
**Password:** Stored in credential vault key `platform_demo_user`; value rotated 2026-05-11.

---

## Resolution Update - 2026-05-11

`platform_demo_user` has been repaired and rotated. The current password is stored only in vault key `platform_demo_user`; do not write it to docs, scripts, logs, or shell history.
...
| Wazuh Dashboard backend | PASS: OpenSearch Security `authinfo` recognizes `platform_demo_user` |
...
### Diagnostic Findings
- `platform_demo_user` EXISTS in GitLab DB (User ID: 4, active, confirmed, admin)
...
`platform_demo_user` was created in the RBAC DB but **NOT in OpenSearch Security**.
...
### Fix Required
1. Add `platform_demo_user` to `internal_users.yml` on the indexer container:
   ```yaml
   platform_demo_user:
...
### Symptom
Wazuh API returns 401 "Invalid credentials" for `platform_demo_user` even though the user exists in the RBAC DB.
...
| Component | wazuh-wui | platform_demo_user |
|-----------|-----------|----------------|
...
docker exec -i itop-deployment-db-1 mariadb -uitop -p"<from container MYSQL_PASSWORD>" itop -e \
  "SELECT pu.login, length(pul.password_hash) as len, substr(pul.password_hash, 1, 15) as prefix \
   ...
   WHERE pu.login = 'platform_demo_user';"

# Expected output: platform_demo_user | 60 | $2b$12$...
```
...
`platform_demo_user` exists in `itop`, `wazuh`, and `gitlab` realms with passwords set.
