...
| Mailcow | `http://192.168.50.222:2581` | Verified | Bare root URL is routed to the admin UI and stale user-session cookies are recovered; login reaches `/admin/dashboard`; dashboard, system, mailbox, queue, and quarantine pages show no invalid JSON, SQL-column warning, or blank-page errors. `/webmail` and `/SOGo/so` render the demo webmail surface backed by real Mailcow IMAP/SMTP. Use `demo_account_1@mailcow.local` and the shared vault password. Report Phish proof: ticket `578`, iTop Incident `370`, quarantine id `28cd6d435f7c88cd9a7b46983c62a1cb`, agent `227`, gate `167`. |
