...
| Mailcow | `http://192.168.50.222:2581` | Verified | Bare root URL is routed to the admin UI and stale user-session cookies are recovered; login reaches `/admin/dashboard`; dashboard, system, mailbox, queue, quarantine, and `/SOGo/so` recovery routes show no invalid JSON, SQL-column warning, or blank-page errors; demo mailbox IMAP auth works as `demo_account_1@mailcow.local`. Use the shared vault password. |
