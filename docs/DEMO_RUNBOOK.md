...
| Ops Chat / Element | `https://192.168.50.222:3303` | Verified | `http://192.168.50.222:3301` redirects here. This same URL also proxies Matrix client/OIDC callback paths, so the browser does not need to connect to `3302` for normal chat. Sign in with Keycloak as `demo_chat_alice`, `demo_chat_jeff`, or `demo_chat_exec`. Direct-message `@agentic-ops:agentic-ops.local`; the bot auto-joins, replies, creates a dashboard ticket for operational work, and spawns a real Hermes agent when policy allows. Latest browser proof marker `ops-chat-same-origin-playwright-1779261056` created ticket `908`, agent `307`, task `304`, final state `awaiting_user_response`. |
