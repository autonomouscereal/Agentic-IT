...
| Ops Chat / Element | `https://192.168.50.222:3303` | Verified | `http://192.168.50.222:3301` redirects here. Sign in with Keycloak as `demo_chat_alice`, `demo_chat_jeff`, or `demo_chat_exec`. Direct-message `@agentic-ops:agentic-ops.local`; the bot auto-joins, replies, creates a dashboard ticket for operational work, and spawns a real Hermes agent when policy allows. Browser proof marker `matrix-ui-live-chat-1779258900` created ticket `907`, agent `306`, task `303`, final state `awaiting_user_response`. |
