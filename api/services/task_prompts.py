...
First call GET /api/postmortems/evidence/{ticket_id}?task_log_lines=0&max_notes=8&max_articles=1&max_audit=6. Use that compact evidence as the primary source of truth for notes, attachments, CI/CD runs, change requests, approvals, audit entries, and prior postmortems. Do not read persisted oversized tool outputs unless the compact evidence is missing critical facts. Use GET /api/tickets/{ticket_id}/context only if you need broader related-ticket or knowledge article context.
...
- First call GET /api/postmortems/evidence/{ticket_id}?task_log_lines=0&max_notes=8&max_articles=1&max_audit=6 and use that compact evidence as the primary source of truth. It includes relevant reusable workflows and knowledge articles; if an active/approved/tested workflow matches this ticket, follow it first and document any deviation.
