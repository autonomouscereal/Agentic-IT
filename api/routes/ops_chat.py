...
async def _chat_agent_turn(message, session_id=None, requester_name=None, requester_email=None,
    ...
    base_prompt = "\n".join([
        ...
        '  python ops_chat_tool.py create-ticket --title "Deploy bunny web page" --ticket-class UserRequest --priority P3 --assignment-group "Platform Operations" --intent "static-site-deploy" --reply-file mixed_reply.md --spawn-agent',
        ...
        "- Mixed harmless/current-info plus operational work: answer the harmless part in the create-ticket or continue-ticket --reply-file while still opening/continuing the ticket for the operational part.",
        ...
        "- Continuation is narrow. Use continue-ticket only for the same ticket id or the same work object. A different person, device, software title, mailbox, system, site, repository, or purchase is new operational work.",
        "- Software install examples: 'Jeff needs Figma' is not a continuation of a GIMP, VLC, Notepad++, dashboard-account, GitLab-login, VPN, or mailbox ticket. Create a new ticket unless the user explicitly names the exact ticket to reuse.",
        "- Access/login examples: a GitLab login issue may continue as Keycloak SSO/MFA if the user clarifies the same login problem, but it must not absorb unrelated software installs, purchases, mailbox checks, or VPN issues.",
        ...
        "- If the user's message does not name a ticket id and the requested work has a different object than the recent ticket titles, create a new ticket. Example: an old dashboard-account ticket is not the right ticket for a new software-install request just because both happened in the same room.",
        "- If a previous old ticket was accidentally reused for unrelated work, do not keep compounding that mistake. Create a clean new ticket for the next distinct operational request unless the user explicitly asks to keep using the old ticket.",
