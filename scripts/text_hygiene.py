...
# Keys are written with escapes so this script does not contain the worst bad
# text it is designed to block.
...
ALL_REPLACEMENTS = {**MOJIBAKE_REPLACEMENTS, **ASCII_REPLACEMENTS}
...
def fix_text(text: str, include_lab_values: bool, ascii_only: bool) -> str:
    fixed = text
    replacements = ALL_REPLACEMENTS if ascii_only else MOJIBAKE_REPLACEMENTS
...
def scan_text(path: Path, text: str, include_lab_values: bool, ascii_only: bool):
    findings = []
    markers = sorted(ALL_REPLACEMENTS if ascii_only else MOJIBAKE_REPLACEMENTS)
    for marker in markers:
        if marker in text:
            label = "non-ascii marker" if ascii_only and marker not in MOJIBAKE_REPLACEMENTS else "mojibake marker"
            findings.append(f"{path.relative_to(ROOT)}: contains {label} {marker!r}")
...
def main():
    ...
    parser.add_argument("--ascii", action="store_true", help="Also normalize intentional Unicode punctuation and box drawing.")
    ...
    for path in iter_files():
        ...
        if args.fix:
            fixed = fix_text(text, args.check_lab_values, args.ascii)
            if fixed != text:
                path.write_text(fixed, encoding="utf-8", newline="\n")
                changed.append(path.relative_to(ROOT).as_posix())
            text = fixed

        findings.extend(scan_text(path, text, args.check_lab_values, args.ascii))
